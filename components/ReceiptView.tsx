...
import { PdfIcon, DeleteIcon, Rupee, ViewIcon, WhatsAppIcon, SmsIcon } from './Icons';
...
const ReceiptView: React.FC<ReceiptViewProps> = ({ receipts, onNew, onEdit, onDelete }) => {
    ...
    const handleSendSms = (receipt: Receipt) => {
        if (!receipt.mobileNumber) {
            alert("This receipt does not have a mobile number associated with it.");
            return;
        }
        ...
        const message = `Vanakam from Arulmigu Angala Parameswari Temple. Thank you, ${receipt.devoteeName}! We have received your offering. Receipt No: ${receipt.receiptNumber}. Seva(s): ${itemsList}. Total: ₹${total.toLocaleString('en-IN')}.`;

        const userConfirmed = window.confirm(
            "இது ஒரு SMS அனுப்ப உங்கள் சாதனத்தின் இயல்புநிலை செய்தியிடல் செயலியைத் திறக்கும். அந்த செயலியில் நீங்கள் 'அனுப்பு' என்பதை அழுத்த வேண்டும்.\n\nதொடர விரும்புகிறீர்களா?"
        );
        
        if (userConfirmed) {
            sendSmsMessage(receipt.mobileNumber, message);
        }
    };
                                ...
                                <div className="flex items-center space-x-1">
                                    ...
                                    {receipt.mobileNumber && (
                                        <button 
                                            ...
                                            aria-label="Send SMS Confirmation">
                                            <SmsIcon />
                                        </button>
                                    )}
